...
public class Main {
    ...
    public static void main(String[] args) {
        String test = "A A A B C D E F G X";
        String test3 = "A A A C G G G C C B C D E F G Y J";
        String test2 = "B B B C D E F G G Z";
        String test4 = "A A A Z J";

        System.out.println("=====TF SCORES=====");
        HashMap<String, Double> a = computeTF(test);
        for(Map.Entry<String, Double> x: a.entrySet()) {
            System.out.println(x.getKey() + " has value: " + x.getValue());
        }
        System.out.println("=====IDF SCORES=====");
        String[] old = new String[] {test2, test3, test4};
        HashMap<String, Double> b = computeIDF(old, test);
        for(Map.Entry<String, Double> x: b.entrySet()) {
            System.out.println(x.getKey() + " has value: " + x.getValue());
        }
        System.out.println("=====TF IDF SCORES=====");
        HashMap<String, Double> c = computeTFIDF(a, b);
        for(Map.Entry<String, Double> x: c.entrySet()) {
            System.out.println(x.getKey() + " has value: " + x.getValue());
        }
    }
